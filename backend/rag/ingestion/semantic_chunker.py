...
from rag.embeddings.gemini import Embedder, GeminiEmbedder
...
class SemanticMarkdownChunker:
    ...
    def _split_section(
        ...
        distances = [
            ...
            for left, right in zip(embeddings, embeddings[1:], strict=False)
        ]
    ...
    def _build_candidates(
        ...
        for unit in units:
            ...
            exceeds_maximum = (
                current and current_chars + len(unit.text) > self.max_chunk_chars
            )
            if (
                heading_changed
                or current_chars >= self.candidate_chars
                or exceeds_maximum
            ):
