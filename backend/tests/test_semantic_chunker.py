...
def test_single_oversized_sentence_respects_hard_limit() -> None:
    chunker = SemanticMarkdownChunker(
        FakeEmbedder(), candidate_chars=10, min_chunk_chars=20, max_chunk_chars=100
    )
    markdown = "# Long\n\n" + ("electrical " * 40)

    chunks = chunker.chunk_text(markdown, source="long.md")

    assert all(chunk.char_count <= 110 for chunk in chunks)
